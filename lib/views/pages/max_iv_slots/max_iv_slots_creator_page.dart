import 'package:auto_route/auto_route.dart';
import 'package:breeder/blocks/pages/max_iv_slots_page/a_iv_slots_amount_state.dart';
import 'package:breeder/blocks/pages/max_iv_slots_page/iv_slots_amount_cubit.dart';
import 'package:breeder/shared/router/router.gr.dart';
import 'package:breeder/views/pages/max_iv_slots/max_iv_slots_text_forms_widget.dart';
import 'package:breeder/views/widgets/buttons/custom_text_button.dart';
import 'package:breeder/views/widgets/generic/custom_container.dart';
import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';

// @RoutePage()
class MaxIVSlotsCreatorPage extends StatefulWidget {
  const MaxIVSlotsCreatorPage({Key? key}) : super(key: key);

  @override
  _MaxIVSlotsCreatorPage createState() => _MaxIVSlotsCreatorPage();
}

class _MaxIVSlotsCreatorPage extends State<MaxIVSlotsCreatorPage> {
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<IVSlotsAmountCubit, AIVTextFormsState>(
      builder: (BuildContext context, AIVTextFormsState ivTextFormsState) {
        return CustomContainer(
          containerWidth: 350,
          containerHeight: 650,
          columnItems: <Widget>[
            Container(
              margin: const EdgeInsets.only(top: 30, left: 30, right: 30, bottom: 25),
              child: const Text(
                'Enter the quantity of monsters with maximum IVs',
                textAlign: TextAlign.center,
                style: TextStyle(
                  fontSize: 20,
                  fontWeight: FontWeight.w400,
                ),
              ),
            ),
            const MaxIVSlotsTextFormsWidget(),
            SizedBox(
              width: 300,
              child: BlocBuilder<IVSlotsAmountCubit, AIVTextFormsState>(
                // bloc: ivSlotsAmountCubit,
                builder: (BuildContext context, AIVTextFormsState ivTextFormsState) {
                  return Center(
                    child: Row(
                      children: <Widget>[
                        CustomTextButton(
                          buttonText: 'Back',
                          icon: Icons.navigate_before,
                          leftMargin: 25,
                          onPressed: () => AutoRouter.of(context).push(const NewBreedingRoute()),
                        ),
                        CustomTextButton(
                          buttonText: 'Cancel',
                          icon: Icons.cancel,
                          leftMargin: 25,
                          onPressed: () => AutoRouter.of(context).push(const MainMenuRoute()),
                        ),
                        CustomTextButton(buttonText: 'Next', icon: Icons.navigate_next, leftMargin: 25, onPressed: () {}),
                      ],
                    ),
                  );
                },
              ),
            ),
          ],
        );
        // ),
        // ),
        // ),
      },
    );
  }
}
